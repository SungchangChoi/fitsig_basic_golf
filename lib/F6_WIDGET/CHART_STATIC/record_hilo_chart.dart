...
class RecordHiloChart extends StatelessWidget {
  final double width;
  final double height;
  ...
  final List<String>? lineNameList;
  final String unit;
  final bool isTrendLineOn;

  const RecordHiloChart({
    this.width = 200,
    this.height = 200,
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: BasicRecordHiloChart(
        ...
        gridLineColor: Colors.transparent, // tm.grey02,
        ...
        // ------------------------------- x-axis title
        yAxisTitle: yAxisTitle!=null?AxisTitle(
            text: yAxisTitle,
            textStyle: TextStyle(
                color: tm.grey03,
                fontSize: tm.s12
        ...
        // 그래프 data range 가 변할때 호출되는 callback 함수. (그래프에 현재 보여지는 data index 값의 min, max 을 double 형태로 얻을 수 있는음)
        ...
        xAxisLabelFormatter: (AxisLabelRenderDetails axisLabelRenderArgs) {
          ...
          TextStyle style = TextStyle(color: tm.grey04, fontSize: tm.s12, fontWeight: FontWeight.bold);
          if (gvRecord.graphTimePeriod.value == GraphTimePeriod.aWeek) {
            // 1일일 경우, 월만 붙임
            if (day == 1) {
              text = '$month월';
              // style = TextStyle(color: tm.grey04, fontSize: tm.s14, fontWeight: FontWeight.w500);
            } else {
              text = '$day일';
            }
        ...
        // 각각의 label 에 대해서 적용을 하다보니, 어떤 label 은 분, 어떤 라벨은 시간 단위로 표시되게됨
        yAxisLabelFormatter: (AxisLabelRenderDetails axisLabelRenderArgs) {
          String text = axisLabelRenderArgs.text;
          if (axisLabelRenderArgs.value == 40){
            text = '${axisLabelRenderArgs.text}$unit';
          }
          TextStyle style = TextStyle(color: tm.grey03, fontSize: tm.s12, fontWeight: FontWeight.bold);
          return ChartAxisLabel(text, style);
        },
